...
step_dotfiles() {
    ...
    # Generate monitors.conf if it doesn't exist (machine-specific)
    if [ ! -f "$HOME/.config/hypr/monitors.conf" ]; then
        info "Generating monitors.conf with default monitor config..."
        cat > "$HOME/.config/hypr/monitors.conf" << 'MONCONF'
# Generated by installer - edit with nwg-displays or manually
# Format: monitor=NAME,WIDTHxHEIGHT@RATE,PosX,PosYPie,Scale
monitor=,preferred,auto,1
MONCONF
        echo "  CREATE: monitors.conf (auto-detect)"
    fi
